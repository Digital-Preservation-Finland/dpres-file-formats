    ...
    if active or dps_spec_formats:
        for file_format in file_formats:
            ...
            for version in file_format['versions']:
                dps_format = any((
                    dps_spec_formats and version['added_in_dps_spec'],
                    not dps_spec_formats))
                version_active = any((active and version['active'],
                                      not active))
...
def mimetypes_grading(text_formats=False):
    ...
    for file_format in file_formats:
        ...
        for version in file_format['versions']:
            if version['active']:
                format_versions[version['version']] = version['grade']
        ...
        if format_versions:
            ...
            if any((not text_formats,
                    text_formats and file_format['charsets'])):
...
def mimetypes_format_registry_keys():
    ...
    for file_format in file_formats:
        ...
        for version in file_format['versions']:
            if version['active']:
                active = True
                format_registry_keys.add(version['format_registry_key'])
        ...
        if active:
            ...
            if file_format['mimetype'] in mimetypes:
                mimetypes[file_format['mimetype']].update(format_registry_keys)
            else:
                mimetypes[file_format['mimetype']] = format_registry_keys
...
def containers_streams_grading(grade=None):
    ...
    for container_dict in containers:
        ...
        streams = set()
        for stream in container_dict['audio_streams']:
            streams.add((stream['mimetype'], stream['version']))
        for stream in container_dict['video_streams']:
            streams.add((stream['mimetype'], stream['version']))

        if any((grade and container_dict['grade'] == grade,
                not grade)):
